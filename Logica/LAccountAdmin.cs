...
namespace Logica
{
    public class LAccountAdmin
    {
        private DaoAccountAdmin daoAccountAdmin;

        public LAccountAdmin(VialtecContext context)
        {
            daoAccountAdmin = new DaoAccountAdmin(context);
        }

        public LAccountAdmin() { }

        public async Task<DistributorUser> Login(string email, string passKey)
        {
            passKey = MD5Hash(passKey);
            return await daoAccountAdmin.Login(email, passKey);
        }
